...
git clone https://github.com/NIKO/NIKO.git
cd NIKO
chmod +x linuxNIKO.sh
./linuxNIKO.sh --build
...
git clone https://github.com/NIKO/NIKO.git
cd NIKO
chmod +x linuxNIKO64.sh
./linux64NIKO.sh --build

Options:
--reset: update dependencies, clone NIKO repository
--force: if you are using other versions such as Debian version > 7

Example with options:
./linux64NIKO.sh --reset --force --build

After successful build, NIKOd can be found in NIKO/src/
