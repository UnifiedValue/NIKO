...
{
    // Convert NIKO:// to NIKO:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("NIKO://"))
    {
        uri.replace(0, 12, "NIKO:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "NIKO.lnk";
}
...
{
    return GetAutostartDir() / "NIKO.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("NIKO-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  NIKO-qt [" + tr("command-line options") + "]                     " + "\n";
